...
func camMenuOn() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x06, 0x06, 0x02, 0xFF})
}

func camMenuOff() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x06, 0x06, 0x03, 0xFF})
}

func camMenuToggle() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x06, 0x06, 0x10, 0xFF})
}

func camMenuUp() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x03, 0x01, 0x01, 0xFF})
	time.Sleep(time.Millisecond*100)
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x03, 0x01, 0x00, 0xFF})
}

func camMenuDown() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x03, 0x02, 0x01, 0xFF})
	time.Sleep(time.Millisecond*100)
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x03, 0x02, 0x00, 0xFF})
}

func camMenuLeft() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x01, 0x03, 0x01, 0xFF})
	time.Sleep(time.Millisecond*100)
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x01, 0x03, 0x00, 0xFF})
}

func camMenuRight() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x02, 0x03, 0x01, 0xFF})
	time.Sleep(time.Millisecond*100)
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x02, 0x03, 0x00, 0xFF})
}

func camMenuSetEnter() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x07, 0x01, 0x01, 0xFF})
	time.Sleep(time.Millisecond*100)
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x07, 0x01, 0x00, 0xFF})
}

func camMenuCancelBack() () {
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x07, 0x01, 0x01, 0xFF})
	time.Sleep(time.Millisecond*100)
	sendVisca(camPort, []byte{(0x80+8), 0x01, 0x7E, 0x04, 0x40, 0x07, 0x01, 0x00, 0xFF})
}
